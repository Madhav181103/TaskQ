...
async function dequeueJob() {
  // Define a Lua script to atomically pop the highest priority job (lowest score) 
  // and record it in the processing hash. This guarantees atomicity on the server,
  // eliminates the dequeue-to-processing race condition window, and supports 
  // older Redis versions (such as Redis 3.x on Windows) that lack ZPOPMIN.
  const luaScript = `
  ...
  const id = await redis.eval(luaScript, 2, QUEUE_KEY, PROCESSING_KEY, startTime);
  if (!id) {
    return null;
  }
